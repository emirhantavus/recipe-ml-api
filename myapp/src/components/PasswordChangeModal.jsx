...
function PasswordChangeModal({ isOpen, onClose }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();//butona basınca sayfa yenilenmesin
    setError("");
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => {
        onClose();//şifre değiştirme pop kapatılır
        setSuccess(false);
        setOldPassword("");//eski bilgileri sıfırlar
        setNewPassword("");
        setConfirmPassword("");
      }, 2000);
    } catch (error) {
  ...
  if (!isOpen) return null;//Açık değilse bir şey gösterme
